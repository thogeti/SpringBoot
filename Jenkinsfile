        ...
        stage('---package--') {
		  ...
		  //  echo "$SERVICE/pom.xml"
		   ...
		   //// bat "mvn -f MavenHelloWorldProject/pom.xml -Dmaven.test.failure.ignore=true clean package"
		    bat "mvn -f CDR_API_Service/pom.xml -Dmaven.test.failure.ignore=true clean package"
		    bat "mvn -f CDR_Canonical_Model/pom.xml -Dmaven.test.failure.ignore=true clean package"
		    bat "mvn -f CDR_CommonUtil/pom.xml -Dmaven.test.failure.ignore=true clean package"
		    bat "mvn -f CDR_Exception_Framework/pom.xml -Dmaven.test.failure.ignore=true clean package"
		    bat "mvn -f PatientService/pom.xml -Dmaven.test.failure.ignore=true clean package"
                  echo 'Packaging..'
            }
        }
     stage('---Publish--') {
		...
		////    nexusPublisher nexusInstanceId: 'CDR_Central', nexusRepositoryId: 'maven-releases', packages: [[$class: 'MavenPackage', mavenAssetList: [[classifier: '', extension: '', filePath: 'D:\\Git\\MavenHelloWorldProject\\target\\MavenHelloWorldProject-1.0-SNAPSHOT.jar']], mavenCoordinate: [artifactId: 'Drop_60', groupId: 'CDR_Central', packaging: 'jar', version: '$FILENAME+$VERSION']]]
		  ...
		  nexusPublisher nexusInstanceId: 'CDR_Central', nexusRepositoryId: 'maven-releases', packages: [[$class: 'MavenPackage', mavenAssetList: [[classifier: '', extension: '', filePath: 'D:\\Git\\SpringBoot\\DevHotFix\\PatientService\\target\\PatientService-Drop65_20210110.war']], mavenCoordinate: [artifactId: 'Drop65', groupId: 'CDR_central', packaging: 'war', version: '1.0']]]  
            }
        }
    }
}
